...
const REASON_KEY: Record<string, string> = {
  sahte_yaniltici: "admin.reasonSahteYaniltici",
  yanlis_kategori: "admin.reasonYanlisKategori",
  uygunsuz_icerik: "admin.reasonUygunsuzIcerik",
  satildi_kaldirilmadi: "admin.reasonSatildiKaldirilmadi",
  diger: "admin.reasonDiger",
};

const STATUS_KEY: Record<AdminReport["status"], string> = {
  BEKLEMEDE: "admin.reportStatusBeklemede",
  INCELENDI: "admin.reportStatusIncelendi",
  REDDEDILDI: "admin.reportStatusReddedildi",
};
...
export default function AdminReportsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <h1 className="mb-1 font-display text-2xl font-bold">{t("admin.listingReportsTitle")}</h1>
      <p className="mb-5 text-sm text-ink-muted">{t("admin.listingReportsSubtitle")}</p>

      {error && <p className="text-sm text-red-500">{error}</p>}

      {!error && !reports && <p className="text-sm text-ink-muted">{t("common.loading")}</p>}

      {reports && (
        <div className="card overflow-x-auto">
          {reports.length === 0 ? (
            <EmptyState title={t("admin.noReportsYet")} description={t("admin.noReportsDesc")} />
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface2 text-left text-xs uppercase tracking-wide text-ink-muted">
                  ...
                  <th className="px-4 py-2.5">{t("admin.colReason")}</th>
                  <th className="px-4 py-2.5">{t("admin.colDescription")}</th>
                  <th className="px-4 py-2.5">{t("admin.colReporter")}</th>
                  <th className="px-4 py-2.5">{t("common.date")}</th>
                  <th className="px-4 py-2.5">{t("admin.colStatus")}</th>
                  <th className="px-4 py-2.5">{t("admin.colActionSingle")}</th>
                </tr>
              </thead>
              <tbody>
                {reports.map((r) => (
                  <tr key={r.id} className="border-b border-border last:border-0 align-top">
                    <td className="px-4 py-2.5 font-medium">
                      {r.listing ? (
                        ...
                        <span className="text-ink-muted">{t("admin.deletedListing")}</span>
                      )}
                      {r.listing?.isSold && (
                        <span className="ml-1.5 rounded-full bg-surface2 px-1.5 py-0.5 text-[10px] font-semibold text-ink-muted">
                          {t("dealerProfile.soldBadge")}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-2.5">{REASON_KEY[r.reason] ? t(REASON_KEY[r.reason]) : r.reason}</td>
                    <td className="max-w-[220px] px-4 py-2.5 text-ink-muted">{r.message || "—"}</td>
                    <td className="px-4 py-2.5 text-ink-muted">
                      {r.reporter ? r.reporter.fullName || r.reporter.companyName || `@${r.reporter.username}` : t("admin.anonymousLabel")}
                    </td>
