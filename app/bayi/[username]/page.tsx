...
async function getTeamMembers(dealerId: string) {
    const members = await prisma.user.findMany({
          where: { parentDealerId: dealerId, approved: true },
...
export default async function DealerProfilePage({ params }: { params: { username: string } }) {
  ...
  const [listings, teamMembers] = await Promise.all([
        getListings(dealer.id),
        getTeamMembers(dealer.id),
      ]);
                  ...
                  <div className="card mt-6 p-5 sm:p-6">
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                              {teamMembers.map((m) => {
                                  ...
                                  return (
                                                    <div key={m.id} className="flex flex-col items-center gap-2 rounded-lg border border-border p-3 text-center">
                                                                      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-blueprint/10 text-base font-bold text-blueprint">
